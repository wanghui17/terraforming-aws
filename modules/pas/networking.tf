...
resource "aws_subnet" "services_subnets" {
  ...
  tags = "${merge(var.tags, tomap({"Name" = "${var.env_name}-pks-services-subnet${count.index}"}), 
      tomap({"kubernetes.io/role/internal-elb" = "1"}), 
      tomap({"SubnetType" = "Private"}))}"
...
resource "aws_security_group" "pks_internal_security_group" {
  ...
  tags = "${merge(var.tags, tomap({"Name" = "${var.env_name}-pks-internal-security-group"}))}"
}
...
resource "aws_security_group" "pks_master_security_group" {
  ...
  tags = "${merge(var.tags, tomap({"Name" = "${var.env_name}-pks-master-security-group"}))}"
}
