...
data "template_file" "csi" {
  template = "${file("${path.module}/templates/csi_iam_policy.json")}"
}

data "template_file" "antrea" {
  template = "${file("${path.module}/templates/antrea_iam_policy.json")}"
}

data "aws_iam_policy_document" "pks_work_policy" {
  source_policy_documents = [data.template_file.csi.rendered, data.template_file.antrea.rendered]
